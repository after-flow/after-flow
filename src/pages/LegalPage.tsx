...
export function LegalPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl p-4 py-6">
      {/* 同意画面からも、アプリ内のどこからでも開かれる。元いた場所へ戻す。 */}
      <button
        type="button"
        onClick={() => navigate(-1)}
        className="inline-flex items-center gap-1 text-sm font-bold text-[var(--color-brand)] underline"
      >
      ...
      <p className="mb-4 text-sm text-[var(--color-ink-faint)]">最終更新：{doc.updatedAt}</p>

      <Banner tone="warning" title="この文面は確定していません">
        本文は仮置きです。弁護士による作成・確認を経て差し替えます。
        <strong>【要確定】</strong>と記載した箇所は、収益モデルや専門家紹介の対価など、
        方針が決まっていない事項です。
      </Banner>
        ...
        {doc.sections.map((sec) => (
          <Card key={sec.heading} title={sec.heading}>
            <div className="flex flex-col gap-2">
              {sec.body.map((p, i) => (
                <p key={i} className={p.startsWith('【要確定】') ? 'text-[var(--color-state-yellow)]' : ''}>
                  {p}
                </p>
              ))}
            </div>
