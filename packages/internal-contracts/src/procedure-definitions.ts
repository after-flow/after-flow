...
export const STRUCTURAL_FIELDS = {
  case: [], profile: [], persons: [], relationships: ['fromPersonId', 'toPersonId'],
...
export const procedureDefinitionSchema = z.object({
  ...
  guidance: z.object({
    ...
    /** persons を誰について投影するか。申請者要件を扱う手続きは実行者本人に限定する。 */
    personScope: z.enum(['all-active', 'initiating-member']).default('all-active'),
...
type ProcedureDefinitionInput = z.input<typeof procedureDefinitionSchema>
type RawDefinition = Omit<ProcedureDefinitionInput, 'version' | 'reviewStatus'> & Partial<Pick<ProcedureDefinitionInput, 'version' | 'reviewStatus'>>
...
export const PROCEDURE_DEFINITIONS: readonly ProcedureDefinition[] = [
  ...
  define({ id: 'pension-stop', title: '年金の受給停止の手続きをする', summary: '受給していた年金の種類に応じた受給停止の届出を案内する。',
    guidance: { requiredContext: [P.pension], optionalContext: [P.dueDate], researchScope: scope('pension-office', '日本年金機構', ['nenkin-death-procedures']),
  ...
  define({ id: 'unpaid-pension-claim', title: '未支給年金を請求する', summary: '未支給年金の請求資格・提出先・必要書類・期限を案内する。',
    guidance: { requiredContext: [P.pension, P.relationshipLabel], optionalContext: [P.dueDate], researchScope: scope('pension-office', '日本年金機構', ['nenkin-death-procedures']),
  ...
  define({ id: 'survivor-pension-check', title: '遺族年金を受け取れるか確かめる', summary: '遺族基礎年金・遺族厚生年金の受給要件と請求方法を案内する。',
    guidance: { requiredContext: [P.pension, P.relationshipLabel], optionalContext: [P.isHeir], researchScope: scope('pension-office', '日本年金機構', ['nenkin-death-procedures']),
  ...
  define({ id: 'inheritance-choice', title: '相続の方法を決める（承認・放棄の判断）', summary: '単純承認・相続放棄・限定承認の選択に必要な情報を整理する。',
    guidance: { requiredContext: [P.knownAt, P.isHeir],
      ...
      researchScope: scope('court', '裁判所', ['inheritance-renunciation']), questions: ['選択肢と効果は何か', '判断期限と起算日は何か', '調査不足時の対応は何か', '専門家確認が必要な条件は何か'] } }),
  define({ id: 'inheritance-renunciation', title: '相続放棄を申述する', summary: '対象相続人の相続放棄申述について、管轄・期限・必要書類を案内する。',
    guidance: { requiredContext: [P.knownAt, P.jurisdictionMunicipality, P.isHeir, P.decisionMethod, P.decisionState],
      optionalContext: [P.relationshipKind, P.assetConfirmation, P.liabilityConfirmation, P.dueDate], researchScope: scope('family-court', null, ['inheritance-renunciation']),
  ...
  define({ id: 'final-income-tax-return', title: '準確定申告をする', summary: '死亡年の所得税について申告要否・提出方法・必要資料を整理する。',
    guidance: { requiredContext: [P.dateOfDeath, P.knownAt, P.isHeir], optionalContext: [P.jurisdictionMunicipality, P.occupation, P.dueDate], researchScope: scope('tax-office', '国税庁', ['final-income-tax-return']),
  ...
  define({ id: 'inheritance-tax-return', title: '相続税の申告が必要か確かめる', summary: '相続人・財産・債務・分割状況から申告要否と申告準備を整理する。',
    guidance: { requiredContext: [P.dateOfDeath, P.isHeir, P.assetKind, P.assetAmount, P.assetConfirmation],
      ...
      researchScope: scope('tax-office', '国税庁', ['inheritance-tax-return']), questions: ['申告要否は何で判定するか', '提出先と期限は何か', '評価・債務控除の資料は何か', '未分割時の扱いは何か'] } }),
  ...
  define({ id: 'real-estate-registration', title: '不動産の相続登記をする', summary: '不動産の所在地・取得原因に応じた相続登記を案内する。',
    guidance: { requiredContext: [P.realEstate, P.isHeir, P.decisionMethod, P.decisionState], optionalContext: [P.assetKind, P.assetAmount, P.dueDate], researchScope: scope('legal-affairs-bureau', null, ['real-estate-registration']),
 ...
 * id, version, allowlist フィールド, STRUCTURAL_FIELDS だけを残す。
 * 複合値はここで安全な形へ縮退し、確認者・確認メモなどを AI Context へ持ち込まない。
...
export function projectGuidanceContext(def: ProcedureDefinition, source: GuidanceProjectionSource): GuidanceProjection {
  ...
  const projectedByGroup = new Map<ContextGroup, Record<string, unknown>[]>()
  const safeValue = (group: ContextGroup, field: string, value: unknown): unknown => {
    if ((group === 'assets' || group === 'liabilities') && field === 'confirmation') {
      const parsed = z.object({ state: z.enum(['CONFIRMED', 'UNCONFIRMED']) }).passthrough().safeParse(value)
  ...
  const isKnown = (value: unknown): boolean => value !== null && value !== undefined
  ...
  const projectEntity = (group: ContextGroup, entity: Record<string, unknown>, identity: { id: unknown; version: unknown }): Record<string, unknown> => {
    ...
    for (const [field, value] of Object.entries(entity)) {
      ...
      // 除外判定は投影前に使い、時刻そのものは Context に含めない。
      if ((group === 'persons' || group === 'relationships') && field === 'excludedAt') continue
      if (allowedFields?.has(field)) {
        const safe = safeValue(group, field, value)
        if (safe === undefined) { droppedKeys.add(`${group}.${field}`); continue }
        projected[field] = safe
        if (isKnown(safe)) usedKeys.add(`${group}.${field}`)
  ...
  projectedByGroup.set('case', [content.case])
  if (source.profile) {
    const profile = projectEntity('profile', source.profile, caseIdentity)
    if (allow.has('profile')) { content.profile = profile; projectedByGroup.set('profile', [profile]) }
  }
  for (const [group, entities] of Object.entries(source.entities) as [EntityContextGroup, readonly Record<string, unknown>[] | undefined][]) {
    ...
    const activeEntities = group === 'persons' || group === 'relationships'
      ? entities.filter(entity => entity.excludedAt === null || entity.excludedAt === undefined)
      : entities
    const projected = activeEntities.map(entity => projectEntity(group, entity, { id: entity.id, version: entity.version }))
    if (allow.has(group)) { content[group] = projected; projectedByGroup.set(group, projected) }
  }
  const requiredByGroup = new Map<ContextGroup, ContextRequirement[]>()
  for (const requirement of def.guidance.requiredContext) {
    const requirements = requiredByGroup.get(requirement.group) ?? []
