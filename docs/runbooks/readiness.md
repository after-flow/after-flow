...
**`ai_connectivity`が確認できるのはAI Serverプロセスがlivenessへ応答することだけ。** Mastra、OrcaRouter、AI Runtime、Worker、対象operationの機能的な準備完了は保証しない（#123対象外）。AI Server自身の実行compositionは`GET /internal/v1/ready`で分けて公開しているが、Backend readinessの`ai_connectivity`は現在そこまで検査しない。
...
- Mastra、OrcaRouter、AI Runtime、Worker、対象operationの機能的な準備完了はこのreadinessでは確認しない（#123対象外）。
