...
> **履歴資料:** これは2026-09-21時点のスナップショットです。IssueやPRの当時の状態を保存しているため、現在の実装状況には [AI Server README](README.md)、接続条件には [OrcaRouter接続](ORCAROUTER.md)、評価結果には [AIの評価](EVALUATION.md) を使用してください。

今回の作業は機能本体を優先した。実装全体の完了ではなく、業務資料・Backend契約に依存する接続が残っている。開発ComposeではAPIキーがある場合に既定mainへRuntime/Workerを接続し、キーが無い場合は実行受付503を維持する。healthの200を利用可能の証明にはせず、readyを分離している。
...
| Issue | 残る内容 | 必要な前提 |
|---|---|---|
| #47 #48 | OrcaRouterゲートウェイ接続とMastra実疎通は実装済み。Backend証跡・必須利用判定の照合が残る | [接続仕様と検証範囲](ORCAROUTER.md)、主催要項 |
| #53 #54 | Orca経由の実モデルを実装済み。提供先grant・実価格/保持条件・2系列の実切り替えが残る | 最低2 Providerの選定、許可範囲、Backend grant契約、設定済み資格情報 |
...
| #65 | ハッカソン用entrypointとAI専用ローカルRuntimeは接続済み。Backend worker・画面・production設定を含むE2Eが残る | Backend #122、AI専用クラウド設定 |

これらを未実装のままIssueを閉じない。#66〜#72の評価拡張は今回の主作業にしていない。実装の検証には型/Lint/通常テスト/OpenAPI/buildと必要なFirestore Emulator試験を実施した。OrcaRouterの実疎通と合成データによる試験の範囲は接続仕様に分けて記載する。業務品質・実OCRは未検証。
