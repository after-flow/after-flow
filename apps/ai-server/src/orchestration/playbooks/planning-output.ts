...
export function validatePlan(input: { runId: string; context: CoreContext; draft: PlanningDraft; templates: readonly ReviewedTaskTemplate[]; sources: readonly SourceDocument[] }): ValidatedPlan {
  ...
  for (const selection of draft.tasks) {
    ...
    if ([...tasks.values()].some(task => normalize(task.get('title')) === normalize(template.task.title) && (!task.get('submitTo') || normalize(task.get('submitTo')) === normalize(template.task.submitTo)))) {
      plan.skipped.push({ templateId: template.id, reason: 'EXISTING_TASK' }); continue
    }
    if (history.proposals.some(proposal => proposal.kind === 'TASK_PROPOSAL' && normalize(proposal.targetTitle) === normalize(template.task.title)) ||
      history.versions.some(version => normalize(version.targetTitle) === normalize(template.task.title))) {
    ...
    const basis = selection.dependencyTaskIds.map(id => ({ type: 'TASK' as const, id, version: versions.get(id)!, label: String(tasks.get(id)!.get('title') ?? id).slice(0, 120) }))
    const actionId = actionIdFor(input.runId, `planning-${template.version}`, template.id)
    plan.proposals.push({ actionId,
      draft: { kind: 'TASK_PROPOSAL', title: template.task.title, summary: template.task.summary, payload: { ...template.task, dependencyTaskIds: selection.dependencyTaskIds, requiredDocuments: template.requiredDocuments.map(label => ({ id: contentHash({ actionId, label }), label })) }, basis, assetDisposal: false },
