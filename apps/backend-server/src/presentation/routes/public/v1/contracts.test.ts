...
describe('contracts / benefits API', () => {
  ...
  it('records policy via Command with decider and time; CANCEL is only a recorded intent', async () => {
    ...
    assert.equal(same.json.error.code, 'INVALID_TRANSITION')
  ...
  it('reports progress as USER_REPORTED and enforces transitions', async () => {
    ...
    assert.equal(undecidedComplete.json.error.code, 'INVALID_TRANSITION')
  ...
  it('reports benefit progress and replays idempotent requests', async () => {
    ...
    const first = await h.request('POST', url, { body, idempotencyKey: 'p1' })
    assert.equal(first.status, 200)
    const replay = await h.request('POST', url, { body, idempotencyKey: 'p1' })
    assert.equal(replay.status, 200)
    assert.equal(replay.json.data.version, first.json.data.version)
    const reused = await h.request('POST', url, { body: { ...body, expectedVersion: 2 }, idempotencyKey: 'p1' })
  ...
  it('enforces Case membership, roles and Case isolation', async () => {
    const h = createHarness()
    const c = await seedContract(h)
    assert.equal((await h.request('GET', `/cases/${CASE_A}/contracts`, { user: OUTSIDER })).status, 403)
